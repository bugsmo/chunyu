...
import (
	...
	"github.com/bugsmo/cy/utils/crypto"
	"github.com/bugsmo/cy/utils/fieldmaskutil"
	"github.com/bugsmo/cy/utils/trans"
	"github.com/go-kratos/kratos/v2/log"
)

// var _ v1.UserServiceServer = (*UserRepo)(nil)
...
func (r *UserRepo) convertEntToProto(in *ent.User) *v1.User {
	...
	if in.Authority != nil {
		// 使用 trans.Int32ToPointer 函数和 v1.UserAuthority_value 映射将 Authority 枚举值转换为 v1.UserAuthority 类型。
	...
	return &v1.User{
		...
		CreateTime:    trans.TimeToTimestamppb(in.CreateTime),
		UpdateTime:    trans.TimeToTimestamppb(in.UpdateTime),
		DeleteTime:    trans.TimeToTimestamppb(in.DeleteTime),
	}
}
...
func (r *UserRepo) Update(ctx context.Context, req *v1.UpdateUserRequest) error {
	...
	builder.Mutation().SetUpdateTime(time.Now())
